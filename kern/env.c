...
env_init(void)
{
	...
	// zhege xiefa hui chufa "Triple fault", buzhidao weishenme
	/*
	env_free_list = NULL;
	struct Env *FreeHead = env_free_list;
	curenv = envs;
	...
	for(i = 0; i < NENV; i++){
		...
		curenv->env_link = env_free_list->env_link;
		
		env_free_list->env_link = curenv;
		curenv += sizeof(struct Env);
	}*/
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	*/
    lcr3(PADDR(e->env_pgdir));  // 应该是切换页目录

    struct Proghdr* ph = ph = (struct Proghdr* )((uint8_t *)header + header->e_phoff);
...
//
void
env_run(struct Env *e)	// e is the new environment
{
